...
- Enabled support for ONNX-format models by default and changed the CLI,
  examples and documentation to use ONNX versions of existing models
  (https://github.com/robertknight/ocrs/pull/272).

  This change is in preparation for phasing out support for the `.rten` format
  in future releases of the RTen runtime. Models in `.rten` format continue to
  be supported, but a future release of Ocrs will remove this.

## [0.13.0] - 2026-08-29

- Updated to rten v0.26. This improves text recognition speed by 15-20%
  (https://github.com/robertknight/ocrs/pull/260,
  https://github.com/robertknight/ocrs/pull/265). As a consequence, the minimum
  supported Rust version has updated to 1.94.

- Added a C API, enabling use from non-Rust programming languages
  (https://github.com/robertknight/ocrs/pull/257) - thanks @xsawyerx

- Fixed `--clipboard` on Wayland (https://github.com/robertknight/ocrs/pull/255)
  - thanks @bbb651

- Added crate features to allow disabling the WASM API and support for
  `.rten`-format models, reducing binary size
  (https://github.com/robertknight/ocrs/pull/242) - thanks @GICodeWarrior
